: $Id: intf6.mod,v 1.98 2012/03/09 03:44:06 samn Exp $
...
:** vers gives version
PROCEDURE vers () {
  printf("$Id: intf6.mod,v 1.98 2012/03/09 03:44:06 samn Exp $\n")
}
  ...
  for(iCell=0;iCell<pg->cesz;iCell++){
    ...
    for(i=0;i<qp->dvt;i++){
      ...
      if(qp->pdope[i] > -1e9 && d <= maxeligtrdur ) { // -1e9 means it wasn't activated. maxeligtrdur is max time
        ...
        if(qp->wgain[i]<0.) qp->wgain[i]=0.; // check bounds of wgain
        else if(!SOFTSTDP && qp->wgain[i]>maxw) qp->wgain[i]=maxw;
      }
    }
  }
  return 1.0;
