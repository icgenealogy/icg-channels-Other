: $Id: vecst.mod,v 1.476 2009/10/16 15:10:25 billl Exp $
...
NEURON {
  ...
  GLOBAL BVBASE, RES, VECST_INSTALLED, DEBUG_VECST, VERBOSE_VECST, INTERP_VECST
}
...
static double ident (void* vv) {
  ...
  printf("Obj*%x Dbl*%x Size: %d Bufsize: %d\n",(unsigned long int)vv,(unsigned long int)x,nx,bsz);
  return (double)nx;
}
ENDVERBATIM
...
PROCEDURE install_vecst () {
  if (VECST_INSTALLED==1) {
    printf("$Id: vecst.mod,v 1.476 2009/10/16 15:10:25 billl Exp $\n")
  } else {
  VECST_INSTALLED=1
  VERBATIM {
  int i,j; 
