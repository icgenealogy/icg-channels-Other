...
NET_RECEIVE (wAM,wNM,wGA,wGB,wAM2,wNM2,wGA2,wflg) { LOCAL tmp,jcn,id
  ...
  if (scaling && (!ip->inhib || scaleinhib)) {
    // Only update if either scaling switch on, or (cell is not inhib OR we are scaling all I+E cells)
      ...
      if (scaling) {
        for (ii=sy,nsyn=0;ii<sy+2;ii++) {
          if (!ip->inhib) {
            ...
            // Scale I cell
            // Scaling has opposite effects on I cells
            if (ii==AM2 || ii==AM) { // || ii==NM || ii == NM2) {
...
PROCEDURE setIscaling() {
  ...
  // Global procedure which sects a flag to which all cells subscribe
  if (*getarg(1) == 1) {
