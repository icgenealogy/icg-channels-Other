: $Id: vecst.mod,v 1.105 2003/04/02 23:07:40 billl Exp $
...
: comes back down again; places values from tvec in dest
VERBATIM
static double xing(void* vv) {
  int i, j, nsrc, ndest, ntvec, f;
  double *src, *dest, *tvec, th;
  ndest = vector_instance_px(vv, &dest);
  nsrc = vector_arg_px(1, &src); th=0;
  ntvec = vector_arg_px(2, &tvec); th=0;
  th = *getarg(3);
  if (nsrc!=ndest||nsrc!=ntvec) hoc_execerror("v.xing: vectors not all same size", 0);
